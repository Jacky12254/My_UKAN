...
# 构建绝对路径
# src_dir_opt_clear = os.path.join(current_dir, './opt_clear')
src_dir_opt_cloudy = os.path.join(current_dir, './opt_cloudy')
src_dir_sar_VH = os.path.join(current_dir, './SAR/VH')
src_dir_sar_VV = os.path.join(current_dir, './SAR/VV')
...
for image_file_index in tqdm(range(len(image_files_opt_cloudy)), desc='Processing images'):#tqdm是一个进度条库，这里是显示进度条
    # Load the image
    ...
    image_file = image_file_opt_cloudy.replace('.png', '.tif')
    ...
    assert image_opt_cloudy.shape[:2] == image_sar_VH.shape[:2] == image_sar_VV.shape[:2], "所有图片的尺寸必须一致"
    multi_channel_image = np.concatenate((image_opt_cloudy, image_sar_VH, image_sar_VV), axis=-1)
